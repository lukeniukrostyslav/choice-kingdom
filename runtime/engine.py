...
class DecisionEngine:
    ...
    def available(self, state: GameState) -> tuple[Event, ...]:
        if state.terminal:
            return ()
        return tuple(
            event for event in self.catalog.events.values()
    ...
    def activate_delayed_target(self, state: GameState, exactly_once_key: str, *, condition_satisfied: bool | None = None) -> DelayedActivationResult:
        if state.terminal:
        ...
        return DelayedActivationResult(resolved.exactly_once_key, resolved.resolution_target, state.snapshot())

    def execute_delayed_target(self, state: GameState, exactly_once_key: str, choice_id: str, *, condition_satisfied: bool | None = None) -> ExecutionResult:
        if state.terminal:
            raise ValueError("cannot execute a choice after terminal state")
        delay = state.pending_delays.get(exactly_once_key)
        if delay is None:
        ...
        try:
            ...
            restored = GameState.from_snapshot(before)
