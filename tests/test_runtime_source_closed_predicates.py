...
def test_source_closed_predicates_compile_only_from_canonical_facts() -> None:
    ...
    state.flags.update(
        {
            "people_charter_endorsed", "crown_audited", "auditor_independence",
            "legislative_budget_lock", "army_constitution_oath", "systemic_explanation_convergence",
            "coalition_candidate_package", "guild_tribunal_independent", "merchant_charter",
            "guild_neutral_inspectors", "food_logistics_stabilized",
        }
    )
    state.history.update({
        "history.house_assembly", "history.guild_representation",
        "history.guild_logistics_cooperation", "history.cross_faction_package",
    })
...
def test_constitutional_readiness_rejects_supporting_red_line_without_e199_oath() -> None:
    ...
    assert "pred.constitutional_prepared_strong" not in EndingSourceCompiler.compile_state(state).predicates
    state.flags.add("army_constitution_oath")
    assert "pred.constitutional_prepared_strong" in EndingSourceCompiler.compile_state(state).predicates
...
def test_coalition_predicate_rejects_noncanonical_shortcuts() -> None:
    ...
    assert "pred.coalition_cooperation" not in EndingSourceCompiler.compile_state(state).predicates
...
def test_food_predicate_uses_the_explicit_producer_marker() -> None:
    ...
    event = next(event for event in catalog.events.values() if event.trigger.lower().strip().rstrip(".") == "pred.food_stable")
    state.flags.add("food_logistics_stabilized")
    assert "pred.food_stable" in EndingSourceCompiler.compile_state(state).predicates
    assert catalog.trigger_satisfied(event.event_id, state) is True
    state.flags.add("food_logistics_unstable")
    assert "pred.food_stable" not in EndingSourceCompiler.compile_state(state).predicates
