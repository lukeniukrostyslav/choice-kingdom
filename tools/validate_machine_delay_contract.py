...
for row in contract.get("delays",[]):
    source=row.get("sourceEventId",""); target=row.get("resolutionTarget",""); key=row.get("exactlyOnceKey",""); choice=row.get("sourceChoiceId",""); identity=(source,choice,target); earliest=(row.get("earliestTurn") or {}).get("relativeToSource"); actual_delays[identity]=earliest
    ...
    if expected is None:
        if "later military crisis" not in str(row.get("cancellationRule","")).lower(): errors.append("E17-A -> E185 must retain the authored later-military-crisis condition")
        if earliest is not None: errors.append("E17-A -> E185 must be condition-bound with earliestTurn=null")
...
for consumer,(source,choice) in expected_sources.items():
    ...
    if row and row.get("status")!="CLOSED": errors.append(f"canonical graph delay status not CLOSED for {consumer}")
if errors:
    print("MACHINE_DELAY_CONTRACT: FAIL"); [print(f"- {e}") for e in errors]; raise SystemExit(1)
print("MACHINE_DELAY_CONTRACT: PASS"); print(f"delays={len(contract.get('delays',[]))}"); print("identity_set=E181-E185,E242-E246"); print("scope=E01-E272"); print("excluded=E273-E277"); print("save_load_policy=persistent"); print("replay_policy=run_scoped_pending_delay"); print("source_contract=CLOSED"); print("production_runtime=NOT_CLAIMED")
