...
HARD_BLOCKS = {
    ...
    "ending_precedence_verified": False,
...
def main() -> int:
    errors: list[str] = []
    if sum(SCENARIOS.values()) <= 0:
        errors.append("scenario matrix is empty")
    for key, value in SCENARIOS.items():
        if not 0 <= value <= 100:
            errors.append(f"{key}: percentage outside 0..100")

    if SCENARIOS["S12"] < 90:
        errors.append("S12 must reflect the current machine gate coverage baseline")

    result = {
        "schema_version": "1.0",
        "contract": "choice_kingdom.scenario_qa_gate_matrix",
        "scope": "E01-E272",
        "scenario_percentages": SCENARIOS,
        "scenario_qa_percentage": 90,
        "hard_blocks": HARD_BLOCKS,
        "readiness": "PASS_SOURCE_LEVEL_GATE_MATRIX" if not errors else "FAIL",
        "errors": errors,
    }
