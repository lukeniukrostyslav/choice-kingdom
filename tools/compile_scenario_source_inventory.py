...
# Canonical source-language normalization. These are not new facts: each mapping
# points an old/prose trigger form to the exact canonical marker used by the
# authored route or its frozen source contract.
TRIGGER_ALIASES = {
    "schoolhouse_vote": "local_budget_vote",
    "temporary_noble_exemption": "estate_exception",
    "warehouse_arson": "warehouse_investigation_available",
    "border_crisis_declared = true": "border_crisis_declared",
    "thread.endgame_convergence": "pred.final_charter_prerequisites",
}
...
for source in sources:
    ...
    for index, match in enumerate(matches):
        ...
        raw_trigger_tokens = sorted(set(backtick_re.findall(trigger_text)))
        trigger_tokens = sorted({TRIGGER_ALIASES.get(token, token) for token in raw_trigger_tokens})
        choices = []
        for choice in choice_re.findall(block):
            ...
            for token in output_tokens:
                producers[token].append({"event": event_id, "source": source, "choice": choice[:120], "kind": "authored"})
...
# Frozen source-backed derived contracts. They close trigger families whose
# authored prose is explicit but whose marker is intentionally represented in
# the machine contract rather than invented as an ordinary choice flag.
derived_contracts = manifest.get("derived_source_contracts", [])
for contract in derived_contracts:
    fact = contract["fact"]
    producers[fact].append({
        "event": contract.get("event", "DERIVED"),
        "source": contract.get("source", "docs/MACHINE_CANONICAL_GRAPH_01.json"),
        "choice": contract.get("choice", "derived source contract"),
        "kind": "derived_source_contract",
    })
...
inventory = {
    "schema": "choice-kingdom-scenario-source-inventory-3",
    ...
    "canonical_trigger_aliases": TRIGGER_ALIASES,
    "derived_source_contracts": derived_contracts,
    "notes": [
        "Source-level inventory only; no gameplay/fresh-run reachability claim.",
        "Canonical trigger aliases are source-language normalization, not invented runtime facts.",
        "Derived source contracts are admitted only when their source evidence is frozen in MACHINE_CANONICAL_GRAPH_01.json.",
        ...
        "E21-A repeating ledger_fragment_a is an idempotent reaffirmation of the earlier E07-B evidence unlock.",
...
print(f"derived_source_contracts={len(derived_contracts)}")
