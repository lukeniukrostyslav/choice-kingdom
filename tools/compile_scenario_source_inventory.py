...
EXTRA=ROOT/"docs/SCENARIO_DERIVED_PRODUCER_CONTRACTS_02.json"
if not MANIFEST.exists(): print("SOURCE_INVENTORY: FAIL\n- missing canonical graph manifest"); sys.exit(1)
manifest=json.loads(MANIFEST.read_text(encoding="utf-8")); sources=manifest.get("source_of_truth",{}).get("catalog_sources",[])
excluded=set(manifest.get("scope",{}).get("excluded_events",[])); first=int(manifest.get("scope",{}).get("first_event",1)); last=int(manifest.get("scope",{}).get("last_event",272)); expected={f"E{i:02d}" for i in range(first,last+1)}-excluded
heading_re=re.compile(r"^### (E\d{2,3}) — (.+)$",re.M); trigger_re=re.compile(r"^\*\*Trigger:\*\* (.*)$",re.M); backtick_re=re.compile(r"`([^`]+)`"); choice_re=re.compile(r"^- \*\*[AB]\b.*$",re.M); clear_re=re.compile(r"(?:^|\s)(?:clear|clears|cleared|reset|resets|remove|removes|removed|erase|erases|erased|invalidate|invalidates|invalidated|revoke|revokes|revoked|cancel|cancels|cancelled|canceled)\s+(?:the\s+)?$",re.I)
errors=[]; events={}; producers=defaultdict(list); consumers=defaultdict(list); predicate_edges=defaultdict(set); aliases=manifest.get("canonical_trigger_normalizations",{})
for source in sources:
    ...
    for i,m in enumerate(matches):
        ...
        if eid in events: errors.append(f"duplicate authored event heading: {eid}"); continue
        block=text[m.start():matches[i+1].start() if i+1<len(matches) else len(text)]; tm=trigger_re.search(block); trigger=tm.group(1).strip() if tm else ""
        tokens=sorted({aliases.get(t,t) for t in set(backtick_re.findall(trigger))}); choices=[]
        for choice in choice_re.findall(block):
            ...
            for token in outs: producers[token].append({"event":eid,"source":source,"choice":choice[:120],"kind":"authored"})
        for token in tokens: consumers[token].append({"event":eid,"source":source,"trigger":trigger})
        ...
        events[eid]={"event_id":eid,"title":title,"source":source,"trigger":trigger,"trigger_tokens":tokens,"choices":choices}
contracts=list(manifest.get("source_closed_producers",[]))+list(manifest.get("derived_source_contracts",[]))
if EXTRA.exists(): contracts += json.loads(EXTRA.read_text(encoding="utf-8")).get("contracts",[])
for c in contracts:
    producers[c["fact"]].append({"event":c.get("event","DERIVED"),"source":c.get("source","docs/MACHINE_CANONICAL_GRAPH_01.json"),"choice":c.get("choice","source contract"),"kind":"source_contract"})
...
duplicates={t:w for t,w in sorted(producers.items()) if len(w)>1}; same_event_shared={}
...
IDEMPOTENT={("history.guild_logistics_cooperation","E194"),("ledger_fragment_a","E21")}; semantic={t:w for t,w in duplicates.items() if len({x["event"] for x in w})>1 and not any((t,x["event"]) in IDEMPOTENT for x in w)}; reaffirmed={t:[x for x in w if (t,x["event"]) in IDEMPOTENT] for t,w in duplicates.items() if any((t,x["event"]) in IDEMPOTENT for x in w)}
undefined=sorted({t for t in consumers if t not in producers}); nodes=sorted(set(predicate_edges)|{t for t in producers if t.startswith("pred.")}|{t for t in consumers if t.startswith("pred.")}); graph={n:sorted(predicate_edges.get(n,set())) for n in nodes}; cycles=[]; state={}; stack=[]
def visit(n):
