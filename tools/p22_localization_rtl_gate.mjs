...
for(const vp of [{w:360,h:800},{w:412,h:915},{w:1440,h:900}]){
 ...
 await p.goto('http://127.0.0.1:4173/design-v2/p22-localization-rtl-proof.html',{waitUntil:'networkidle'});
 const x=await p.evaluate(()=>{
  ...
  return {base:base(),it,uk,ar,reset,exclusive,noOverflow,responsive:/@media\(max-width:760px\)/.test(s)&&/@media\(max-width:420px\)/.test(s),rtl:/html\[dir=rtl\]/.test(s),safe:/safe-area-inset/.test(s),reduced:/prefers-reduced-motion:reduce/.test(s),focus:/:focus-visible/.test(s),serif:/var\(--ck-serif\)/.test(s),cinematic:/radial-gradient/.test(s)&&/linear-gradient/.test(s),allText:[...document.querySelectorAll('h1,h2,h3,p,button')].every(e=>e.scrollWidth<=e.clientWidth+1||getComputedStyle(e).overflow!=='hidden')};
 });
