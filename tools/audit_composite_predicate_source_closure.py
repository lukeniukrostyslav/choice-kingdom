...
def main()->int:
    contract=CONTRACT.read_text(encoding="utf-8"); registry=REGISTRY.read_text(encoding="utf-8"); audit=AUDIT.read_text(encoding="utf-8"); combined="\n".join((contract,registry,audit)); errors=[]; checked={}
    for predicate,needles in REQUIRED.items():
        missing=[n for n in needles if n not in combined]; checked[predicate]={"required_evidence":needles,"missing_evidence":missing,"source_evidence_present":not missing}
    ...
    for assertion in hard_negatives:
        if assertion not in audit: errors.append(f"missing bounded non-promotion assertion: {assertion}")
    report={"schema_version":"1.1","contract":"choice_kingdom.composite_predicate_source_closure","scope":"E01-E272","readiness":"BLOCKED" if errors else "SOURCE_LEVEL_CLOSED","gameplay_verified":False,"runtime_verified":False,"semantic_equality_proven":False,"errors":errors,"checked":checked,"rules":["source evidence presence does not establish an executable producer","consumer events may not manufacture predicates they consume","E273-E277 are excluded from production semantics","this gate does not infer reachability, lifecycle, scheduling, or invalidation","OPEN/PARTIAL contracts remain OPEN/PARTIAL unless authoritative source semantics change"]}
