...
def main() -> int:
    ...
    referenced = expand_inventory_ids(INVENTORY.read_text(encoding="utf-8"))
    available = catalog_ids(manifest)
    lo = int(manifest["scope"]["first_event"]); hi = int(manifest["scope"]["last_event"])
    ...
    missing_from_catalog = set(referenced - available)
    quarantined_missing = sorted(missing_from_catalog & quarantine, key=lambda x: int(x[1:]))
    unexpected_missing = sorted(missing_from_catalog - quarantine, key=lambda x: int(x[1:]))
    errors: list[str] = []
    if outside_scope: errors.append("inventory references outside frozen scope: " + ", ".join(outside_scope))
    if unexpected_missing: errors.append("inventory references IDs absent from catalog headings and not quarantined: " + ", ".join(unexpected_missing))
    warnings: list[str] = []
    if quarantined_missing: warnings.append("inventory references unresolved catalog headings kept in explicit quarantine: " + ", ".join(quarantined_missing))
    status = "PASS_WITH_QUARANTINE" if not errors and warnings else ("PASS" if not errors else "BLOCKED")
    report = {"schema_version":"1.1","contract":"choice_kingdom.delayed_inventory_scope","scope":f"E{lo:02d}-E{hi:02d}","referenced_event_count":len(referenced),"catalog_event_count":len(available),"referenced_events":sorted(referenced,key=lambda x:int(x[1:])),"outside_scope":outside_scope,"missing_from_catalog":sorted(missing_from_catalog,key=lambda x:int(x[1:])),"quarantined_missing":quarantined_missing,"unexpected_missing":unexpected_missing,"status":status,"semantic_timing_verified":False,"runtime_lifecycle_verified":False,"warnings":warnings,"errors":errors}
