...
### E200 — Merchant Oath
`pred.guild_influence_strong` must be computed from at least **three distinct institutional guild domains**: representation, tribunal, commercial_market, and/or qualified logistics. `rel.ivo` alone is invalid.
...
Until these checks pass, runtime engine work remains bounded by the content-first development order and Block 5 remains open.
