...
| P7 | Event / situation presentation | 65% | Full event surface and state variants |
...
| P25 | Final premium polish / cross-screen QA | 47% | Full visual regression and no unresolved P1–P24 blockers |

**Aggregate P1–P25 estimate: 60.28% (simple arithmetic mean of block estimates).** This is an engineering/design evidence estimate, not a commercial-readiness score.
...
- `runtime/premium_surface_host.py` binds the adaptive presentation projection to the existing Realm/History/People/Investigation/Ending/Settings screen hosts without changing gameplay semantics.
- `tests/test_premium_screen_states.py`, `tests/test_premium_surface_projection.py`, `tests/test_premium_regression_matrix.py` and `tests/test_premium_surface_host.py` verify shared interaction states, adaptive projection, cross-screen invariants and host binding.
...
The current increment moves the adaptive surface layer from an isolated projection into the existing non-mutating journey screen hosts and adds a machine-readable cross-screen regression matrix. Next: bind the same contract to Event/Choice and Consequence hosts, then produce rendered cross-screen evidence. Source-level regression does not replace physical Android screenshot/device proof.
