...
**Systemic qualification contract:** `systemic_explanation_convergence` is authoritative only when, before E270-A, all three independent evidence families are already present in authored state:
1. warehouse/financial evidence;
2. document/language evidence;
3. witness/organizational evidence;
4. the explicit E270-A convergence decision is then recorded as the convergence step.
